...
HipSTRParallel is a performance fork of the original HipSTR source in `HipSTR/src`. The main behavioral goal is to preserve output ordering and file formats while running independent STR regions concurrently.
...
- `snp_bam_processor.*` moves SNP phasing preparation into the pipeline work item and protects shared SNP VCF and haplotype-tracker state with mutexes when those inputs are active.
- `genotyper_bam_processor.*` buffers per-region VCF, log, visualization, stutter, timing, and BAM output into `RegionResult`, then merges counters and writes files in BED order.
- `seq_stutter_genotyper.*` can build VCF records as strings for deferred output and serializes the Cephes binomial CDF call used in allele-bias calculation.
- `HapAligner.*` reuses per-aligner scratch vectors for base-quality arrays and DP matrices, removing repeated hot-path `new[]`/`delete[]` allocations during haplotype alignment.
- `bam_io.*` narrows read-group storage to the tags HipSTR uses directly (`ID`, `SM`, and `LB`).
- `region.h`, `adapter_trimmer.*`, and the remaining touched files are kept compatible with the pipeline interfaces; `adapter_trimmer.*` has no material algorithmic change from the original.
